...
def focused_image_selection(image_stack_path: str, output_directory: str, timepoint_index: int = 0, output_naming = 'focused_t={timepoint:03d}_z={zindex:03d}.tiff') -> None:
  ...
  print("dimensions of czi:", zstack.size)

  #timepoints, channels, zs, height, width = zstack.size # tosses an error, missing one value somehow?
  if len(zstack.size) == 6: # another exception lol
    print("this .czi contains a len 6, and was preprocessed to be individual timepoints most likely!")
    _, _, channels, zs, height, width = zstack.size
    timepoints = 1
  elif len(zstack.size) == 4:
    channels, zs, height, width = zstack.size
    timepoints = 1
  else: # changed structure, was just if else before the additional exception
    # jank to flatten to first time instant
    timepoints, channels, zs, height, width = zstack.size 
    # above is the old call, now flatten
    timepoints = 1 # just take first one to not have a headache and rewrite everything, for now
    print("This czi contained multiple timepoints, flattening to first to not break later code!") 
