...
if __name__=="__main__":
    ...
    # find first nonzero index in data, such that we know when to turn on "control"
    ...
    # Initial guess for parameters (can be far off)
    #        [a  ,  b,   c,   d,   e,   u]
    #guess = [0.8, 10, 0.2, 90, 80, 0.9]  # Ensure positive values
    guess = [1, 20, 0.1, 100, 100, 1]  # Ensure positive values
    #guess = [1, 2, 3, .08, 2, 2]  # Ensure positive values

    # define bounds to avoid a lazy solution
    #bounds = Bounds([0.5, 10, 0.01, 50, 10, 0.5],  # lower bounds
    #                [5, 50, 1, 200, 500, 2])      # upper bounds

    bounds = Bounds([0.01, 0.01, 0.01, 0.01, 0.01, 0.01],  # lower bounds
                    [200, 200, 200, 200, 200, 200])      # upper bounds
    ...
    methodChoice = 'L-BFGS-B'
    result = minimize(cost, guess, args=(timeDat, zDat, A0), method=methodChoice, bounds=bounds)
    ...
    axs[1].plot(timeDat, zDat, 'o', markersize=2, label='Raw Z Data', alpha=0.5)
